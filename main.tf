...
data "external" "build_folder" {
  program = ["bash", "-c", "${path.module}/bin/folder_contents.sh", var.dockerfile_folder]
}
...
resource "null_resource" "build_and_push" {
  ...
  provisioner "local-exec" {
    command = "bash ${path.module}/bin/build.sh ${var.dockerfile_folder} ${var.ecr_repository_url}:${var.docker_image_tag} ${var.aws_region}"
  }
}
